...
const MOVIE_ACCESS = document.querySelectorAll('.btn-access');
...
const accessFunction = (e) => {
	e.preventDefault();
	let colourAccess = e.target.dataset.accesstype;
	let htmlEl = document.getElementById("elHTML");

	htmlEl.className = '';

	for (var i = 0; i < MOVIE_ACCESS.length; i++) {
		MOVIE_ACCESS[i].classList.remove('is-active');
	}

	e.target.classList.add('is-active');

	if (colourAccess !== undefined && colourAccess !== '') {
		htmlEl.classList.add("access-colour-" + colourAccess);
	}
}
...
const init = () => {
	...
	//== set events for a11y anchors
	for (var i = 0; i < MOVIE_ACCESS.length; i++) {
		MOVIE_ACCESS[i].addEventListener('click', accessFunction, false);
	}
};
